...
(function () {
	...
	this.tmpl = function tmpl(str, data) {
		...
		var fn = /^[-a-zA-Z0-9]+$/.test(str) ?
			cache[str] = cache[str] ||
			tmpl(document.getElementById(str)?.innerHTML || TEMPLATES[str]) :
        ...
        with(obj){p.push('${str.replace(/[\r\t\n]/g, " ").split(startTag).join("\t").replace(re1, "$1\r").replace(re2, "',$1,'").split("\t").join("');").split(endTag).join("p.push('").split("\r").join("\\'")}');}return p.join('');
	`);
	...
	render: function (type, panel = false) {
		...
		var fn = function (component, property) {
			return property.input.on('propertyChange', function (event, value, input) {
				...
				if (property.onChange) {
					let ret = property.onChange.bind(property)(element, value, input, component);
				...
				if (property.htmlAttr) {
					let htmlattr;
					if (typeof property.htmlAttr === "function") {
						htmlattr = property.htmlAttr(property);
					} else {
						htmlattr = property.htmlAttr
					}
					oldValue = element.attr(htmlattr);
					...
					if (htmlattr == "class" && property.validValues) {
						...
						if (value) {
							element = element.attr(htmlattr, value);
						} else {
		...
		function handle_property(property) {
			...
			if (property.init) {
				...
				if (value && htmlAttr == "class" && property.validValues) {
		...
		// function extra_input() {
		// 	let name;
		// 	let nameInput = TextInput.init({ key: "custom-attr" });
		// 	nameInput.on("propertyChange", (ev, value) => {
		// 		name = value;
		// 	})
		...
		// 	var row = $(tmpl('vvveb-property', extra));
		// 	section.append(row);

		// 	handle_property(extra);
		// }
		// extra_input()
...
Vvveb.Builder = {
	...
	_loadIframe: function (url) {
		...
		return this.documentFrame.on("load", function () {
			...
			$(window.FrameWindow).on("unload", function () {
	...
	_frameLoaded: function () {
		...
		self.frameBody.on("vvveb.undo.add vvveb.undo.restore", function () {
	...
	_getElementType: function (el) {
		...
		let name = (el.tagName + (componentName ? " - " + componentName : "") + (componentAttribute ? " - " + componentAttribute : ""))
			...
			if (self.highlightEnabled == true && event.target && isElement(event.target) && event.originalEvent) {
				...
				if (self.isResize) {
					...
					if (self.resizeMode == "css") {
						...
						self.selectedEl.css({ width: self.getPercent(width, Vvveb.Builder.frameBody.outerWidth()), height: self.getPercent(height, Vvveb.Builder.frameBody.outerHeight()) });
		...
		self.frameHtml.on("click", function (event) {
			...
			if (Vvveb.Builder.isPreview == false) {
				if (event.target) {
					...
					if (self.allowResizeableElements || Vvveb.component.resizable) {
						$("#select-box").addClass("resizable");
						self.resizeMode = Vvveb.component.resizeMode || "css";
					} else {
	...
	_initDragdrop: function () {
		...
		$('.drag-elements-sidepane ul > ol > li > li').on("mouseup dragend touchend", function () {
			self.isDragging = false;
			$("#component-clone").remove()
			$("#section-actions, #highlight-name, #select-box").show();;
		});
...
Vvveb.CodeEditor = {
	...
	init: function () {
		...
		Vvveb.Builder.documentFrame.on("load", function () { Vvveb.CodeEditor.setValue(); });
...
Vvveb.Gui = {
	...
	saveAjax: function () {
		...
		}, saveUrl).fail(function () {
			displayToast("bg-danger", "Error saving!");
		}, saveUrl).always(function () {
			$(".loading", btn).toggleClass("d-none");
			$(".button-text", btn).toggleClass("d-none");
		});
	},
	...
	viewport: function () {
		let elem = $("#iframe-wrapper").get(0);
		(['mobile', 'tablet', 'desktop']).map(i => elem.classList.remove(i))
		elem.classList.add(this.dataset.view || "desktop");
	},
	...
	newPage: function () {
		...
		newPageModal.modal("show").find("form").off("submit").submit(function (e) {
			...
			return Vvveb.Builder.saveAjax(data.file, data.startTemplateUrl, function () {
...
Vvveb.SectionList = {
	...
	init: function (allowedComponents = {}) {
		...
		$(".sections-list").on("mouseenter", "li[data-section]", function () {
