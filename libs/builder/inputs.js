...
class MultiInput {
	constructor(inputs, defaults) {
		...
		for (let key in this.inputs) {
			...
			this.div.append(element.prepend($(`<label class="col-12 form-label">${key.slice(0, 1).toUpperCase()}${key.slice(1).toLowerCase()}</label>`)));
			if (defaults[key]) {
			...
			element.on("propertyChanged", this.callOnChange);
		}
	}
	...
	callOnChange() {
		let value = {};
		for (let key in this.inputs) {
			value[key] = this.inputs[key].element.attr("value");
		}
		this.div.trigger('propertyChange', [value, this]);
	}

	setValue(value) {
		for (let key in this.inputs) {
			value[key] && this.inputs[key].setValue(value[key]);
		}
	}
			...
			for (let attr of attributes) {
				if (!defaults[i] && item.attr(attr)?.length) {
					if (attr == "innerText" || attr == "innerHTML") {
						inputs[i] = new (window.CodeInput ? window.CodeInput : TextareaInput)({ key: `${target}_${i}` });
